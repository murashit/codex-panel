...
function activeAgentSummary(items: readonly ThreadStreamItem[], activeTurnId: string | null) {
...
describe("execution state uses typed status adapters before rendered text", () => {
  ...
  it("does not overwrite streamed output with an empty completed item", () => {
    ...
    expect(upsertThreadStreamItemById([streamed], completed)[0]).toMatchObject({ output: "partial output", status: "completed" });
  });
});
