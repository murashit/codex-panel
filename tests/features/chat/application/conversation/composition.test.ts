...
import { createConversationTurnActions } from "../../../../../src/features/chat/application/conversation/composition";
...
function resumeThread(stateStore: ReturnType<typeof createChatStateStore>, items: readonly MessageStreamItem[]): void {
  stateStore.dispatch({
...
describe("createConversationTurnActions", () => {
  it("does not attach composer-only active note context when implementing a plan", async () => {
    ...
    const actions = createConversationTurnActions(
      {
        stateStore,
