...
import { contextSummary, rateLimitSummary, statusDetails } from "../../../../../src/features/chat/panel/runtime/status";
...
describe("runtime status presentation", () => {
  it("summarizes context meter state from one runtime snapshot", () => {
    ...
    expect(contextSummary(snapshot)).toMatchObject({
      detail: "0 / 100,000 (0%). No turns in this thread yet.",
      percent: 0,
    });
    expect(
      contextSummary(
      ...
      detail: "1,000 / 100,000 (1%). Latest usage: 1,000 input, 200 output, 50 reasoning. Total: 2,600 tokens.",
    });
    expect(
      contextSummary(
        runtimeSnapshot({
