...
- Keep the user's requested outcomes and material open questions in view throughout the task. Follow-up findings extend or refine that work unless the user changes its scope; resolving the latest example does not close the broader concern.
...
- Review behavior, responsibility boundaries, implementation quality, and regression risk. Changes to responsibility or dependency boundaries, shared-state ownership, or asynchronous operation lifetimes require a fresh read-only subagent for independent design and regression review. Predominantly moving code, passing tests, and the implementer's own review do not waive this requirement. For other changes, use a fresh read-only subagent when independence materially improves confidence; otherwise use a direct second pass. Give the reviewer the requirements, comparison base, and evidence; withhold your preferred conclusion and prior review verdicts until their initial assessment.
...
- Compare the affected design before and after the change, weighing added responsibilities, state, and coordination against what is removed or improved. Apply this to small changes as well as substantial work; the later simplification pass provides a second opportunity to reconsider the structure.
- Before acting on a behavioral finding, establish the reachable user or lifecycle path, the violated contract and its owner, and the material consequence; for structural findings, identify the concrete maintenance activity and cost. A synthetic reproduction proves only the behavior exercised; check that its assumptions match real producers and supported use. Separate demonstrated problems from hypotheses and optional improvements, and record why material findings are addressed, rejected, or unresolved.
- Use findings to reconsider the shared assumption or ownership that allowed the problem. Trace other paths relying on it, and compare a correction at its owner with local guards, including the state and coordination each would add. Resolving one reproduction does not establish that the affected paths have been reviewed; investigate according to consequence and uncertainty rather than enumerating every imaginable edge case.

### Completion And History

- After substantial implementation works and known review findings are resolved, use a fresh subagent for a simplification pass before declaring the task complete. Examine the whole design that realizes the requested behavior, including tests, fixtures, and relevant pre-existing owners and paths outside the diff. Bound the pass by that behavior, not by the current patch or the latest finding; avoid unrelated repository-wide exploration.
- Treat the implemented structure as revisable. Compare viable simpler arrangements of ownership, state, branching, and coordination, rather than justifying each added piece in isolation. Preserve required behavior, but rework the design when that materially improves the whole, then review and validate the result. Briefly record substantive alternatives examined and why they improve or fail to improve the design; do not invent alternatives to fill a report. Leave the structure unchanged when no worthwhile alternative is found. Removing obsolete code is useful but does not replace this comparison. Implementation and test line counts are prompts for investigation, not reduction targets.
- Apply the same scrutiny to test design: identify the behavior or regression each affected case protects and its value beyond existing coverage, and weigh fixtures, mocks, and implementation coupling against that value. Consider strengthening an existing case or parameterizing a shared flow before adding another setup; preserve independently meaningful scenarios and readable failure diagnosis. Passing tests or increased coverage alone do not justify their maintenance cost.
- Check completion against the full requested outcomes and material open questions. Review agreement and passing checks support that assessment but do not replace it. Continue authorized work when a gap remains; distinguish unresolved evidence or a genuine blocker from completed work rather than narrowing the scope to declare success.
...
- Lead the final report with the resulting behavior and any material limitations, explicitly identifying UI additions, changes, and removals. Summarize verification and its coverage, who independently reviewed what and with what outcome, and the simplification decision. Keep delivery explanations distinct from copy that belongs in the product or its user documentation.
