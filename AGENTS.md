...
- For feature additions, fixes, and refactoring, compare the whole affected design before and after: weigh removed responsibilities, state, branching, and indirection against added concepts and coordination in light of the required behavior. Consider whether existing owners and shared paths or a simpler structure can meet the requirements while preserving appropriate boundaries, and retain added complexity only when the improvement justifies it. Treat implementation and test line-count changes as prompts for comparison, not size limits or reduction targets.
