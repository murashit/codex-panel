...
Run `release:preflight` after the release commit is on `main`; it validates the release commit, installs the exact lockfile dependencies with `npm ci` using the project `.npmrc`, fails if `npm audit --omit=dev --audit-level=low` finds a known vulnerability in a runtime dependency or cannot complete the audit, and runs the same checks used by the release workflow. Do not rely on an existing `node_modules` directory or on `npm ci --dry-run`.
